function aircraftPB() {
  ...
  aircraft.x += localZAxis.x * aircraft.speed.z * gameSpeed;
  aircraft.y += localZAxis.y * aircraft.speed.z * gameSpeed;
  aircraft.z += localZAxis.z * aircraft.speed.z * gameSpeed;

  //移動（縦)
  let localYAxis = getLocalYAxis(aircraft.axis, aircraft.rad);
  aircraft.x += localYAxis.x * aircraft.speed.y * gameSpeed;
  aircraft.y += localYAxis.y * aircraft.speed.y * gameSpeed;
  aircraft.z += localYAxis.z * aircraft.speed.y * gameSpeed;

  //移動（横）
  let localXAxis = getLocalXAxis(aircraft.axis, aircraft.rad);
  aircraft.x += localXAxis.x * aircraft.speed.x * gameSpeed;
  aircraft.y += localXAxis.y * aircraft.speed.x * gameSpeed;
  aircraft.z += localXAxis.z * aircraft.speed.x * gameSpeed;
  ...
  if (aircraft.rad !== 0 || aircraft.rotationSpeed.x !== 0) {
    let newAxisAndRad = returnToAxisAndRad(calcQuaternion(makeQuaternion(getLocalXAxis(aircraft.axis, aircraft.rad), -aircraft.rotationSpeed.x * gameSpeed), makeQuaternion(aircraft.axis, aircraft.rad)));
  ...
  if (aircraft.rad !== 0 || aircraft.rotationSpeed.y !== 0) {
    let newAxisAndRad = returnToAxisAndRad(calcQuaternion(makeQuaternion(getLocalYAxis(aircraft.axis, aircraft.rad), -aircraft.rotationSpeed.y * gameSpeed), makeQuaternion(aircraft.axis, aircraft.rad)));
    aircraft.axis = newAxisAndRad[0];
    aircraft.rad = newAxisAndRad[1];
  }
  ...
  if (aircraft.rad !== 0 || aircraft.rotationSpeed.z !== 0) {
    let newAxisAndRad = returnToAxisAndRad(calcQuaternion(makeQuaternion(getLocalZAxis(aircraft.axis, aircraft.rad), -aircraft.rotationSpeed.z * gameSpeed), makeQuaternion(aircraft.axis, aircraft.rad)));
    aircraft.axis = newAxisAndRad[0];
    aircraft.rad = newAxisAndRad[1];
  }
}